...
public final class ValidationUtils {
    ...
    public static final Integer MIN_AGE_REQUIRED = 18;
    /**
    ...
    public static final String URL_ERROR_MESSAGE =
        "Website URL must start with 'https://' then contain a domain name such as '.com'";
    ...
    public static LocalDate validateDate(
        ...
        if (checkMinimumAge) {
            LocalDate latestAllowedBirthDate = today.minusYears(
                MIN_AGE_REQUIRED
            );

            if (value.isAfter(latestAllowedBirthDate)) {
                throw new ValidationException(
                    "Age must be at least " + MIN_AGE_REQUIRED + " years old."
                );
            }
        } else {
    ...
    private static <
        NumericType extends Number & Comparable<NumericType>
    > NumericType validateNumber(
        NumericType value,
        String fieldName,
        boolean isRequired,
        NumericType min,
